...
- **性能模式**（关动画 + 放慢刷新；低配/省流/低电量可自动开启；快捷键 `P`）
- **延迟趋势火花图**（最近成功探测样本）+ 系统 CPU/内存/磁盘趋势
- **网络质量徽章**（优/良/一般/差）与探测汇总
- **ICMP → TCP443 → TCP80** 级联探测；失败连败计数
- **宿主机 / 容器视角**自动识别，主机名可复制
- 底栏居中 / 紧凑底栏；刷新间隔 3/5/10s；导出 JSON
- 后端定时采集并缓存；多客户端 **不会** 重复启动 Ping 任务
- 首页 **ETag**；`/api/status`、`/api/summary`、`/health`
